...
def generate_transaction_data(num_pks=6, base_timestamp=None):
    ...
    function_names= [
        # 기본 제어
        "turnOn", "turnOff", "setPower",
        # 연결 관리
        "pair", "unpair", "connect", "disconnect", "bind", "unbind",
        # 상태 확인
        "getStatus", "checkConnection", "checkBattery",
        # 시스템 동작
        "reset", "restart", "update", "configure", 
        # 설정 변경
        "setVolume", "setBrightness", "setTemperature"
    ]
    result = []
...
def save_to_csv(data, filename='interaction_data.csv'):
    ...
    filepath = os.path.join(current_dir, "..", "..", "ai", "data", filename)
